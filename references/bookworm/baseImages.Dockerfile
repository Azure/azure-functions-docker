...
ARG DOCKER_REVISION
FROM mcr.microsoft.com/dotnet/sdk:8.0-bookworm-slim-amd64 AS host-builder-dotnet8
ARG HOST_VERSION
ARG EXTENSION_BUNDLE_VERSION_V2
ARG EXTENSION_BUNDLE_VERSION_V3
ARG EXTENSION_BUNDLE_VERSION_V4
...
FROM scratch as linuxbookworm-workers
COPY --from=host-builder-dotnet8 ["/workers", "/workers"]

FROM mcr.microsoft.com/dotnet/aspnet:8.0-bookworm-slim-amd64 AS linuxbookworm-base
ARG HOST_VERSION
ARG DOCKER_REVISION
...
COPY --from=host-builder-dotnet8 [ "/azure-functions-host", "/azure-functions-host" ]
COPY --from=host-builder-dotnet8 [ "/FuncExtensionBundles", "/FuncExtensionBundles" ]
