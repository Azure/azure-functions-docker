...
FROM mcr.microsoft.com/mirror/docker/library/python:3.9-slim-bullseye AS python39-builder
...
COPY --from=workerImage-dotnet8 [ "/workers/python/3.9/LINUX", "/azure-functions-host/workers/python/3.9/LINUX" ]
COPY --from=workerImage-dotnet8 [ "/workers/python/worker.config.json", "/azure-functions-host/workers/python" ]
COPY --from=python39-builder [ "/usr", "/usr" ]
COPY --from=python39-builder [ "/lib", "/lib" ]
COPY --from=python39-builder [ "/lib64", "/lib64" ]
