...
FROM mcr.microsoft.com/mirror/docker/library/python:3.10-slim-bullseye AS python310-builder
...
COPY --from=workerImage-dotnet8 [ "/workers/python/3.10/LINUX", "/azure-functions-host/workers/python/3.10/LINUX" ]
COPY --from=workerImage-dotnet8 [ "/workers/python/worker.config.json", "/azure-functions-host/workers/python" ]
COPY --from=python310-builder [ "/usr", "/usr" ]
COPY --from=python310-builder [ "/lib", "/lib" ]
COPY --from=python310-builder [ "/lib64", "/lib64" ]
