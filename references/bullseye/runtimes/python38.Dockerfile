...
FROM mcr.microsoft.com/mirror/docker/library/python:3.8-slim-bullseye AS python38-builder
...
COPY --from=workerImage-dotnet8 [ "/workers/python/3.8/LINUX", "/azure-functions-host/workers/python/3.8/LINUX" ]
COPY --from=workerImage-dotnet8 [ "/workers/python/worker.config.json", "/azure-functions-host/workers/python" ]
COPY --from=python38-builder [ "/usr", "/usr" ]
COPY --from=python38-builder [ "/lib", "/lib" ]
COPY --from=python38-builder [ "/lib64", "/lib64" ]
