...
FROM mcr.microsoft.com/mirror/docker/library/python:3.7-slim-bullseye AS python37-builder
...
COPY --from=workerImage-dotnet8 [ "/workers/python/3.7/LINUX", "/azure-functions-host/workers/python/3.7/LINUX" ]
COPY --from=workerImage-dotnet8 [ "/workers/python/worker.config.json", "/azure-functions-host/workers/python" ]
COPY --from=python37-builder [ "/usr", "/usr" ]
COPY --from=python37-builder [ "/lib", "/lib" ]
COPY --from=python37-builder [ "/lib64", "/lib64" ]
